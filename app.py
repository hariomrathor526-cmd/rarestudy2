...
def encode_params(batch_id, subject_id=None, chapter_id=None, url=None, parent_id=None, child_id=None):
    ...
    return encrypted_params.replace('/', '_').replace('+', '-').replace('=', '')
...
async def process_chapters(session, chapter_id, batch_id, subject_id, content_type, headers):
    ...
    tasks = [process_content(session, batch_id, subject_id, item["_id"], item['content_type'], headers) for item in all_schedule]
...
# MODIFIED: Key generation/session verification completely removed here
@app.before_request
def ensure_token():
...
def get_batches():
    try:
        ...
        if search:
            filtered_batches = [batch for batch in all_batches if search in batch.get('name', '').lower()]
...
def get_favourite_batches():
    try:
        ...
        return jsonify({'success': True, 'batches': ordered_batches})
